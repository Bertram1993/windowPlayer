...
namespace DuiLib {
	...
	void CComboWnd::Init(CComboUI* pOwner)
	{
		...
		for (int it = 0; it < pOwner->GetCount(); it++) {
			...
			cyFixed += sz.cy;//原本没-8，因为修改了字体大小，无法自适应，所以-8
		}
		cyFixed += 2; // CVerticalLayoutUI 默认的Inset 调整，原本为+4，肖东改为+2
		if (cyFixed > 260)cyFixed = 260;
	...
	LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		if (uMsg == WM_CREATE) {
			...
			m_pLayout->SetBkColor(0xff233D3D);
			//m_pLayout->SetBorderColor(0xFF59CCDA);
			m_pLayout->SetBorderSize(1);
			m_pLayout->SetAutoDestroy(false);
			m_pLayout->EnableScrollBar(1, 0);
			m_pLayout->GetVerticalScrollBar()->SetFixedWidth(5);
			m_pLayout->GetVerticalScrollBar()->SetNormalImage(L"file='web/滚动条背景.png' corner ='6,5,6,5'");
			m_pLayout->GetVerticalScrollBar()->SetThumbNormalImage(L"file='web/滚动条原始.png' corner='6,5,6,5'");
			m_pLayout->GetVerticalScrollBar()->SetThumbHotImage(L"file='web/滚动条经过.png' corner='6,5,6,5'");
			m_pLayout->GetVerticalScrollBar()->SetShowButton1(false);
			m_pLayout->GetVerticalScrollBar()->SetShowButton2(false);
	...
	//肖东添加，修改下拉框的样式，包括上面第118、119行代码，要使用Duilib原始的构造函数直接注释这段然后打开下一个构造函数即可
	CComboUI::CComboUI() : m_pWindow(NULL), m_iCurSel(-1), m_uButtonState(0), m_bWndVisible(false)
	{
		m_szDropBox = CSize(0, 10);
		::ZeroMemory(&m_rcTextPadding, sizeof(m_rcTextPadding));

		m_ListInfo.nColumns = 0;
		m_ListInfo.nFont = 1;
		m_ListInfo.uTextStyle = DT_VCENTER;
		m_ListInfo.dwTextColor = 0xFF59CCDA;
		m_ListInfo.dwBkColor = 0;
		m_ListInfo.bAlternateBk = false;
		m_ListInfo.dwSelectedTextColor = 0x59CCFFFF;
		m_ListInfo.dwSelectedBkColor = 0x7D9AA9;
		m_ListInfo.dwHotTextColor = 0x59CCFFFF;
		m_ListInfo.dwHotBkColor = 0x7D9AA9;
	...
	//肖东注释，下面注释的这一段代码为原始的构造函数，要用直接放出来，然后将上面的构造函数注释即可
	...
	void CComboUI::DoEvent(TEventUI& event)
	{
		...
		{
			if (m_pWindow != NULL && m_bWndVisible == true)
			{
