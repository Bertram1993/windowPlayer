...
namespace DuiLib
{
	...
	void CTreeNodeUI::DoEvent(TEventUI& event)
	{
		...
		if (event.Type == UIEVENT_DBLCLICK) {
			if (IsEnabled()) {
				m_pManager->SendNotify(this, _T("treeitemdbclick"));
				Invalidate();
			}
			return;
		}
		...
		{
			m_pManager->SendNotify(this, _T("treeitemrclick"));
			return;
		}
	...
	{
		TNotifyUI* pMsg = (TNotifyUI*)param;
		if (_tcsicmp(pMsg->sType, _T("treeitemdbclick")) == 0) {
	...
	// 函数说明: 
	//************************************
	void CTreeNodeUI::SetToolTip(LPCTSTR pstrValue)
	{
		GetTreeNodeHoriznotal()->SetToolTip(pstrValue);
	}
}
